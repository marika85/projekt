...
// ---------------------------------------------------------------------------------
    ...
    if (not(kurs_waluta == "PLN")) goto skip_no_dew01
      if (pustepole["c:iz_866"]) goto skip_no_dew011
        ...
        xkurs_kwota := round[stringnaliczbe[rejwezp_s["c:iz_868"]]*dewprzelkurs[xkurs_waluta],4]
    ...
    skip_no_dew01:
      ...
      st := ToStr["DEW#st_r#S = #kwota_dew#S:2,#xkurs_waluta#S:2,#xkurs_kwota#S:4,"]
      FinnLine[st]
      finnop["konieclinii",""]
    skipno_dew:
// ---------------------------------------------------------------------------------
...
//
          ...
          no_kom02:
      if (strin["EUR",kurs_waluta] > -1) kurs_waluta := "EUR"
      if (strin["USD",kurs_waluta] > -1) kurs_waluta := "USD"
      if (strin["GBP",kurs_waluta] > -1) kurs_waluta := "GBP"
          st := ToStr["DEW#st_r#S = #kwota_dew#S:2,#kurs_waluta#S:2,#kurs_kwota#S:4,''"]
          FinnLine[st]
          tr_skipno_dew:    
        nie_rozpisz:
//          st := ToStr["""#tresc_ks#L80"",#konto_klient#S,#kwota#S:X,#konto_ks#S,"]
...
// ------ walutowa linia
      ...
      if (strin["GBP",kurs_waluta] > -1) kurs_waluta := "GBP"
        ...
        if (pustepole["d:it_kwota_zap_zl"]) st := ToStr["DEW#st_r#S = #kwota_dew#S:2,#kurs_waluta#S:2,#kurs_kwota#S:4,"]
        if (not(pustepole["d:it_kwota_zap_zl"])) st := ToStr["DEW#st_r#S = #rejwezp_k[""d:it_kwota_zap""]#S:2,#kurs_waluta#S,#kurs_kwota#S:4,"]
        FinnLine[st]
        finnop["konieclinii",""]
        tskipno_dew:
// ------ 
